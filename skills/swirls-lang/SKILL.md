...
| 4 | Node Types | CRITICAL | `node-` | All 18 node types, required/optional fields, secrets map, failure policy |
...
- `node-database` - Database nodes (`type: database`): governed mutations against a Swirls-managed `database` block; `operation` narrows the injected client to query/insert/update/delete/transaction, enforced at runtime
...
- `context-db` - `context.db.<name>` in `@ts` blocks: full client in `code` nodes and `migration` bodies, operation-narrowed in `database` nodes; `$transaction` only under `operation: transaction`; `return await` support; Date/BigInt/bytes/Decimal round-tripping
